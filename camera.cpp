...
#include <opencv2/highgui.hpp>  //cv::Mat
...
static  void* WorkThread(void* pUser)
{
    ...
    while(1)
    {
        ...
        if(NULL != stOutFrame.pBufAddr)
        {
            ...
            cv::Mat matt = cv::Mat(stOutFrame.stFrameInfo.nHeight, stOutFrame.stFrameInfo.nWidth, CV_8UC3, pDataForRGB);
            cv::resize(matt,matt,cv::Size(1920,1080));
            cv::imshow("1",matt);
            cv::imwrite("1.png", matt);
            cv::waitKey(30);
