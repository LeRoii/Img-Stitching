...
int main()
{
    ...
    *************************************************************************/   
    ...
    for( int i = 1;  i <= image_count ; i++)
    {
        ...
        imageFileName += ".png";
        cv::Mat image = imread("../img/2-"+imageFileName); 
        if(image.empty())
        {
            printf("image empty\n");
        }
    ...
    *************************************************************************/   
    ...
    cv::Mat stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors;                   /* 每幅图像的平移向量 */
    int flags = 0;
    // flags |= cv::fisheye::CALIB_RECOMPUTE_EXTRINSIC;
    // flags |= cv::fisheye::CALIB_CHECK_COND;
    // flags |= cv::fisheye::CALIB_FIX_SKEW;
    cv::calibrateCamera(object_Points, corners_Seq, image_size, intrinsic_matrix, distortion_coeffs, rotation_vectors, translation_vectors, stdDeviationsIntrinsics, stdDeviationsExtrinsics, perViewErrors);
    ...
    *************************************************************************/
    ...
    for (int i = 0 ; i < successImageNum ; i++)
    {
        ...
        // cv::undistort(image_Seq[i], t, intrinsic_matrix, distortion_coeffs, optMatrix);
        string imageFileName;
        std::stringstream StrStm;
